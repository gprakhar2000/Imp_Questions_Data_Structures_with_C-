...
int knapsack(int* weight, int* value, int n, int maxWeight,int **output){
	...
	if(weight[n-1]>maxWeight){
        ans=knapsack(weight,value,n-1,maxWeight,output);
    }
    else{
        ans=max(value[n-1]+knapsack(weight,value,n-1,maxWeight-weight[n-1],output),
            knapsack(weight,value,n-1,maxWeight,output));
